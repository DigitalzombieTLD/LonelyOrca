...
namespace LonelyOrca
{
    ...
    {
        public OrcaTravel(IntPtr intPtr) : base(intPtr)
        {
        }
        ...
        private void DoBreathe()
        {
            ...
            this.delay = UnityEngine.Random.Range(BREATHE_MIN_DELAY, BREATHE_MAX_DELAY);
        }
        ...
        private void DoDive()
        {
            ...
            this.delay = UnityEngine.Random.Range(DIVE_MIN_DELAY, DIVE_MAX_DELAY);
            this.transform.position = this.target;

            if (IsOrcaActive() && IsPlayerCloseEnough())
            {
                this.remainingBreaths = UnityEngine.Random.Range(1, 5);
            }
        }
        ...
        private void DoJump()
        {
            ...
            this.delay = UnityEngine.Random.Range(JUMP_MIN_DELAY, JUMP_MAX_DELAY);
        }
        ...
        private bool GetRandomPosition(out Vector3 target)
        {
            ...
            {
                target = new Vector3(UnityEngine.Random.Range(minX, maxX), WATER_LEVEL, UnityEngine.Random.Range(minY, maxY));
            ...
            MelonLogger.Msg("Failed to create random position inside area");
        ...
        private bool GetTarget(float requiredTravelDistance, out Vector3 target)
        {
            ...
            for (int i = 0; i < MAX_TARGET_FINDING_LOOPS; i++)
            {
                Quaternion randomRotation = Quaternion.AngleAxis(UnityEngine.Random.value * maxRotation, Vector3.up);
           ...
           MelonLogger.Msg("Failed to create target position inside area");
            target = new Vector3(0, WATER_LEVEL, 0);
            return false;
        }
